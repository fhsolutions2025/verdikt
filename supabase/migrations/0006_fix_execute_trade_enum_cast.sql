-- Fix execute_trade: add ::order_status casts to CASE expression
-- The orders.status column is type order_status (enum); text literals
-- need explicit casts or PostgreSQL raises ERROR 42804.
...
begin
  ...
  if v_maker_order.id is not null then
    ...
    set shares_filled = shares_filled + v_shares,
        status = case
          when shares_filled + v_shares >= shares then 'filled'::order_status
